...
export default function ApiClients() {
  ...
  const [scopes, setScopes] = useState<Array<"tenant:identify" | "resource:resolve" | "events:read" | "tenant:list" | "resource:write">>([
    "tenant:identify",
    "resource:resolve",
  ]);
  ...
  const columns: ColumnDef<ApiClient>[] = [
    ...
    },
    {
      ...
      id: "scopes",
      filterFn: (row, _id, value) => (value as Array<"tenant:identify" | "resource:resolve" | "events:read" | "tenant:list" | "resource:write">).some((scope) => row.original.scopes.includes(scope)),
  ...
  function startEdit(client: ApiClient) {
    ...
    setName(client.name);
    setScopes(client.scopes as Array<"tenant:identify" | "resource:resolve" | "events:read" | "tenant:list" | "resource:write">);
  ...
  function toggleScope(scope: "tenant:identify" | "resource:resolve" | "events:read" | "tenant:list" | "resource:write") {
        ...
        <DataTable labels={dataTableLabels} table={table}>
          <DataTableToolbar
            ...
            facets={[
              ...
              { columnId: "scopes", multiple: true, options: ["tenant:identify", "resource:resolve", "events:read", "tenant:list", "resource:write"].map((value) => ({ value, label: value })), title: t("apiClients.filterScope") },
          ...
          {!token ? (
            <>
              ...
              <fieldset className="space-y-2">
                ...
                <label className="flex items-start gap-2 rounded-lg border p-3 text-sm" htmlFor="api-client-scope-resource-write">
                  <Checkbox aria-label={t("apiClients.scopeResourceWrite")} id="api-client-scope-resource-write" checked={scopes.includes("resource:write")} className="mt-0.5" onCheckedChange={() => toggleScope("resource:write")} />
                  <span><strong>{t("apiClients.scopeResourceWrite")}</strong><span className="block text-muted-foreground">{t("apiClients.scopeResourceWriteDescription")}</span></span>
                </label>
              </fieldset>
