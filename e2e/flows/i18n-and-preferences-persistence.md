...
1. (`auth`) Autenticar-se com `konvario_admin_dev` → visão geral carrega.
...
6. (`overview`) Recarregar a página no navegador (refresh) → o idioma e o tema escolhidos permanecem (persistidos em `konvarioLocale` e `konvarioTheme`).
...
- As preferências são guardadas em `localStorage` sob `konvarioLocale` e `konvarioTheme`.
