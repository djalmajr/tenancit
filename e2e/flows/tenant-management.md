...
4. (`tenant-detail`) Clicar em **Editar**, alterar nome ou status e clicar em **Salvar** → o cabeçalho do tenant reflete os dados persistidos.
5. (`tenant-detail`) Clicar na aba **Domínios** → a tabela ou estado vazio de domínios fica visível.
6. (`tenant-detail`) Clicar em **Adicionar**, preencher Hostname e clicar em **Adicionar** → o hostname aparece na tabela de domínios.
...
8. (`tenant-detail`) Voltar ao detalhe e remover o domínio criado pelo botão de lixeira → o domínio deixa de aparecer e o estado vazio informa que ao menos um domínio é necessário para resolver o tenant.
...
- Remoção de domínio não tem confirmação dedicada; validar se isso é aceitável para uma ação destrutiva.
