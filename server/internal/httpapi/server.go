...
func (s *Server) Routes(staticHandler http.Handler) http.Handler {
	...
	r.Group(func(cr chi.Router) {
		...
		cr.With(RequireAPIClientScope(service.ScopeResourceWrite), EnforceAPIClientRateLimit(s.Limiter, s.Usage, "write", s.Now), RecordAPIUsage(s.Usage, "write", s.Now)).Patch("/v1/tenants/{slug}/resources/{alias}/values/{fieldKey}", s.handleConsumerWriteValue)
	})
