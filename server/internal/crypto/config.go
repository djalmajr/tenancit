...
//	TENANCIT_AES_KEY_V<n>    optional older keys (base64) for rotation
func FromEnv() (*Cryptor, error) {
	...
	for v := 1; v < version; v++ {
		if raw := os.Getenv(fmt.Sprintf("TENANCIT_AES_KEY_V%d", v)); raw != "" {
			old, err := base64.StdEncoding.DecodeString(raw)
			if err != nil {
				return nil, fmt.Errorf("crypto: TENANCIT_AES_KEY_V%d not valid base64: %w", v, err)
			}
			keys[v] = old
		}
	}
